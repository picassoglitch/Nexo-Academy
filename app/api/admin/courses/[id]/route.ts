  ...
  try {
    ...
    const course = await prisma.course.update({
      ...
      data: {
        ...
        description,
        published,
        requiredTiers: requiredTiers !== undefined ? requiredTiers : undefined,
      },
      include: {
        modules: {
          include: {
    ...
    return NextResponse.json(
      { error: "Error al actualizar el curso", message: error?.message },
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Error al eliminar el curso", message: error?.message },
      { status: 500, headers: { "Content-Type": "application/json" } }
    )
  }
}
